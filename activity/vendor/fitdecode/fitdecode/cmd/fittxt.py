...
def txt_encode(obj):
    ...
    if isinstance(obj, fitdecode.FitChunk):
        return PrintableObject(index=obj.index, offset=obj.offset, size=len(obj.bytes))
...
def parse_args(args=None):
    ...
    options.filter, options.default_filter = parse_filter_args(parser, options.filter)
...
def main(args=None):
    ...
    try:
        with fitdecode.FitReader(
            ...
            for frame in fit:
                if options.nodef and frame.frame_type == fitdecode.FIT_FRAME_DEFINITION:
                    continue
                ...
                if options.filter and frame.frame_type in (
