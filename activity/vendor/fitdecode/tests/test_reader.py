...
class FitReaderTestCase(unittest.TestCase):
    ...
    def test_fitparse_component_field_accumulaters(self):
        csv_fp = open(_test_file("compressed-speed-distance-records.csv"), mode="rt")
        ...
        for count, (
            record,
            (timestamp, heartrate, speed, distance, cadence),
        ) in enumerate(zip(records, csv_file)):
            ...
            self.assertEqual(
                record.get_field("cadence").value,
                int(cadence) if cadence != "null" else None,
            )
