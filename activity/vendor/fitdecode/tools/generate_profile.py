...
MESSAGE_NUM_DECLARATIONS = ("file_id", "developer_data_id", "field_description", "hr")
...
FIELD_NUM_DECLARATIONS = ("hr.event_timestamp", "hr.event_timestamp_12")

SPECIAL_FIELD_DECLARATIONS = (
    "FIELD_TYPE_TIMESTAMP = Field(name='timestamp', type=FIELD_TYPES['date_time'], def_num="
    + str(FIELD_NUM_TIMESTAMP)
    + ", units='s')"
)
...
IGNORE_TYPE_VALUES = (
    ...
    "date_time:min",
)  # TODO: How to account for this? (see Profile.xls)
...
class TypeInfo(namedtuple("TypeInfo", ("name", "base_type", "enum", "comment"))):
    def get(self, value_name):
        ...
        raise AssertionError(
            "Invalid value name %s in type %s" % (value_name, self.name)
        )
    ...
    def __str__(self):
        ...
        if self.enum:
            ...
            for value in sorted(
                self.enum,
                key=lambda x: x.value if isinstance(x.value, int) else int(x.value, 16),
            ):
                s += "        %s\n" % (value,)
            s += "    },\n"
        s += ")"
...
class MessageList(namedtuple("MessageList", ("messages"))):
    ...
    def get_field_by_name(self, mesg_name, field_name):
        ...
        raise ValueError(
            'field "%s" not found in message "%s"' % (field_name, mesg_name)
        )


class MessageInfo(
    namedtuple("MessageInfo", ("name", "num", "group_name", "fields", "comment"))
):
    def get(self, field_name):
        for field in self.fields:
            if field.name == field_name:
                return field
        raise AssertionError(
            "Invalid field name %s in message %s" % (field_name, self.name)
        )
...
class ComponentFieldInfo(
    namedtuple(
        "ComponentFieldInfo",
        ("name", "num", "scale", "offset", "units", "bits", "bit_offset", "accumulate"),
    )
):
    def __str__(self):
        s = "ComponentField(\n"
...
class SubFieldInfo(
    ...
    def __str__(self):
        ...
        for (
            ...
            self.ref_fields
        ):  # sorted(self.ref_fields, key=lambda rf: (rf.name, rf.value)):
            s += "        %s,\n" % indent(ref_field, 2)
        s += "    ),\n"
        if self.components:
            s += "    components=(\n"
...
class ReferenceFieldInfo(
    namedtuple("ReferenceFieldInfo", ("name", "value", "num", "raw_value"))
):
    def __str__(self):
        s = "ReferenceField(\n"
        s += "    name='%s',\n" % self.name
...
def parse_types(types_rows):
    ...
    for row in types_rows:
        if row[0]:
            ...
            type = TypeInfo(
                name=row[0].decode(),
                base_type=row[1].decode(),
                enum=[],
                comment=row[4].decode(),
            )
...
def parse_messages(messages_rows, type_list):
    ...
    for row in messages_rows:
        if (row[3] is not None) and all(
            r == b"" for n, r in enumerate(row[:14]) if n != 3
        ):
            ...
            message = MessageInfo(
                name=name,
                num=type_list.get_mesg_num(name),
                group_name=group_name,
                fields=[],
                comment=row[13].decode(),
            )
            ...
            if component_names and (
                len(component_names) != 1 or component_names[0] != ""
            ):
                ...
                components = [
                    ComponentFieldInfo(
                        name=cmp_name,
                        num=None,
                        scale=fix_scale(cmp_scale),
                        offset=cmp_offset,
                        units=fix_units(cmp_units),
                        bits=cmp_bits,
                        bit_offset=None,
                        accumulate=bool(cmp_accumulate),
                    )
            ...
            if row[1] is not None and row[1] != b"":
                field = FieldInfo(
                    name=row[2].decode(),
                    type=row[3].decode(),
                    num=maybe_decode(row[1]),
                    scale=fix_scale(row[6]),
                    offset=maybe_decode(row[7]),
                    units=fix_units(row[8].decode()),
                    components=[],
                    subfields=[],
                    comment=row[13].decode(),
                )
                ...
                subfield.ref_fields.extend(
                    ReferenceFieldInfo(
                        name=ref_field_name,
                        value=ref_field_value,
                        num=None,
                        raw_value=None,
                    )
                    for ref_field_name, ref_field_value in zip(
                        ref_field_names, parse_csv_fields(row[12].decode(), 0)
                    )
                )
            ...
            for sub_field in field.subfields:
                for n, ref_field_info in enumerate(sub_field.ref_fields[:]):
                    ...
                    sub_field.ref_fields[n] = ref_field_info._replace(
                        ...
                        raw_value=type_list.get(ref_field.type)
                        .get(ref_field_info.value)
                        .value,
                    )
                bit_offset = 0
...
def get_xls_and_version_from_zip_or_xlsx(path):
    ...
    for name in ("Profile.xlsx", "Profile.xls"):
        ...
        for name in archive.namelist():
            rem = re.fullmatch(
                r"^FitSDKRelease_(\d+(?:\.\d+)+)[/\\]Profile.xlsx?$", name, re.I
            )
            if rem:
                profile_xls = archive.open(name)
    ...
    if profile_version is None:
        ...
        rem = re.search(
            r"\s+Profile\s+Version\s*\=\s*(\d+(?:\.\d+)+)[^\d]*", content, re.I
        )
        if not rem:
...
def main(input_xls_or_zip, output_py_path=None):
    if output_py_path and os.path.exists(output_py_path):
        if (
            not open(output_py_path, "rb")
            .read()
            .strip()
            .startswith(PROFILE_HEADER_FIRST_PART.encode())
        ):
            print("Python file does not begin with appropriate header.")
            sys.exit(1)

    if zipfile.is_zipfile(input_xls_or_zip):
        xls_file, profile_version = get_xls_and_version_from_zip_or_xlsx(
            input_xls_or_zip
        )
    ...
    for mesg_name in MESSAGE_NUM_DECLARATIONS:
        ...
        mesg_num_declarations.append(
            'MESG_NUM_%s = %s  # message "%s"'
            % (
    ...
    output = (
        "\n".join(
            [
                ...
                header(
                    "EXPORTED PROFILE FROM %s ON %s"
                    % (
                        (
                            ("SDK VERSION %s" % profile_version)
                            if profile_version
                            else "SPREADSHEET"
                        ),
                ...
                header(
                    "PARSED %d TYPES (%d VALUES), %d MESSAGES (%d FIELDS)"
                    % (
                        len(type_list.types),
                        sum(len(ti.enum) for ti in type_list.types),
                        len(message_list.messages),
                        sum(len(mi.fields) for mi in message_list.messages),
                    )
                ),
                "",
                IMPORT_HEADER,
    ...
    if output_py_path:
        ...
        print(
            "Profile version %s written to %s"
            % (profile_version if profile_version else "<unknown>", output_py_path)
        )
    else:
        print(output.strip())
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(
            "Usage: %s <FitSDK.zip | Profile.xls> [profile.py]"
